...
class KnightMoves {
    ...
    public calculateMoves(x: number, y: number, currentBoard: number[][]): number {
        let total = 0;
        // ups
    ...
    public update(x: number, y: number, currentBoard: number[][]): void {
        this.up_left = this.inRange(x-1) && this.inRange(y+2) && currentBoard[x-1][y+2] == 0 ? this.calculateMoves(x-1, y+2, currentBoard) : null;
        this.up_right = this.inRange(x+1) && this.inRange(y+2) && currentBoard[x+1][y+2] == 0 ? this.calculateMoves(x+1, y+2, currentBoard) : null;
        this.down_left = this.inRange(x-1) && this.inRange(y-2) && currentBoard[x-1][y-2] == 0 ? this.calculateMoves(x-1, y-2, currentBoard) : null;
        this.down_right = this.inRange(x+1) && this.inRange(y-2) && currentBoard[x+1][y-2] == 0 ? this.calculateMoves(x+1, y-2, currentBoard) : null;
        this.left_up = this.inRange(x-2) && this.inRange(y+1) && currentBoard[x-2][y+1] == 0 ? this.calculateMoves(x-2, y+1, currentBoard) : null;
        this.left_down = this.inRange(x-2) && this.inRange(y-1) && currentBoard[x-2][y-1] == 0 ? this.calculateMoves(x-2, y-1, currentBoard) : null;
        this.right_up = this.inRange(x+2) && this.inRange(y-1) && currentBoard[x+2][y-1] == 0 ? this.calculateMoves(x+2, y-1, currentBoard) : null;
        this.right_down = this.inRange(x+2) && this.inRange(y+1) && currentBoard[x+2][y+1] == 0 ? this.calculateMoves(x+2, y+1, currentBoard) : null;
    }
    ...
    public getMoves(isFirst:boolean): Vector[]|null {
        ...
        const validMoves = moves.filter(x => x[0]);
        if(!validMoves) {
            return null;
        }
        ...
        return isFirst ? validMoves.map(x => x[1]) : validMoves.filter(x => x[0]??0 === minMoves).map(x => x[1])
    }
}

// Recursively walk the board
function walkBoard(x: number, y: number, currentBoard:number[][], path: Vector[], winningPaths: Vector[][]): void {
    // Record move
    ...
    // Exit condition
    if(!nextMoves) {
        if(newBoard.filter(x => x.filter(y => y === 0))) {
    ...
    for(const chosenMove of nextMoves) {
        ...
        walkBoard(x + (chosenMove.x??0), y + (chosenMove.y??0), newBoard, newPath, winningPaths);
    }
}
...
walkBoard(1, 0, board, initialPath, winningTours);
